                            ...
                            <li class="nav-item dropdown dropdown-large">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <div class="header-message-list">
                                       @if(count($posts_comments ?? []) > 0)
    @for ($i = 0; $i < min(10, count($posts_comments)); $i++)
        <a class="dropdown-item" href="{{ route('posts.show', $posts_comments[$i][0]->id ) }}">
            <div class="d-flex align-items-center">
                <div class="user-online">
                    <img class="img_admn--user img-avatar" width="50" height="50" style="border-radius: 50%; margin: auto; background-size: cover; background-image: url({{ $posts_comments[$i][0]->comments()->orderBy('id','DESC')->take(1)->first()->user->image ? asset('storage/' . $posts_comments[$i][0]->comments()->orderBy('id','DESC')->take(1)->first()->user->image->path) : asset('storage/placeholders/user_placeholder.jpg') }})" alt="">
                </div>
                <div style="margin-left: 10px;" class="flex-grow-1">
                    <h6 class="msg-name">{{ $posts_comments[$i][0]->comments()->orderBy('id','DESC')->take(1)->first()->user->name }}<span class="msg-time"> đã bình luận bài viết</span></h6>
